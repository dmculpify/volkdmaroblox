...
bool Memory::Init(std::string process_name, bool memMap, bool debug)
{
	if (!DMA_INITIALIZED)
	{
		...
		const char* required_dlls[] = { "vmm.dll", "leechcore.dll", "FTD3XX.dll" };
		bool missing_dlls = false;
		for (const char* dll : required_dlls)
		{
			if (!std::filesystem::exists(dll))
			{
				LOG("[!] WARNING: Missing required file: %s\n", dll);
				missing_dlls = true;
			}
		}
		if (missing_dlls)
		{
			LOG("[!] Some required DLL files are missing!\n");
			LOG("[!] Make sure vmm.dll, leechcore.dll, and FTD3XX.dll are in the same folder\n");
		}
	...
	reinit:
		...
		if (!this->vHandle)
		{
			...
			LOG("[!] VMMDLL_Initialize failed!\n");
			LOG("[!] Possible causes:\n");
			LOG("[!]   1. DMA card not connected or not detected\n");
			LOG("[!]   2. Missing DLL files (vmm.dll, leechcore.dll, FTD3XX.dll)\n");
			LOG("[!]   3. FTD3XX driver not installed\n");
			LOG("[!]   4. DMA card already in use by another process\n");
			LOG("[!]   5. Wrong device string (currently using: fpga://algo=0)\n");
			LOG("[!] Try running with -v -printf flags for more details\n");
			return false;
		}
	...
	bool find = false;
	for (int p : pidList) {
		this->current_process.PID = p;
		std::vector<std::string> list = GetModuleList(process_name);
		for (std::string str : list) {
			printf(str.c_str());
			printf("\n");
			if (strcmp(str.c_str(), "7z.dll") == 0) {
				find = true;
				break;
			}
		}
	...
	printf("this->current_process.PID = " + this->current_process.PID);
	...
	if (!memory.FixCr3())
	{
		...
		std::cout << "[!]   - DMA card cannot read target system memory properly" << std::endl;
		std::cout << "[!]   - Process '" << process_name << "' module not found" << std::endl;
		std::cout << "[!]   - Target system may have protections enabled" << std::endl;
		std::cout << "[!] Continuing anyway, but memory reads may fail..." << std::endl;
	}
	else
	{
...
uintptr_t Memory::GetExportTableAddress(std::string import, std::string process, std::string module)
{
	...
	bool result = VMMDLL_Map_GetEATU(memory.vHandle, memory.GetPidFromName(process) /*| VMMDLL_PID_PROCESS_WITH_KERNELMEMORY*/, (LPSTR)module.c_str(), &eat_map);
	if (!result)
	{
...
uintptr_t Memory::GetImportTableAddress(std::string import, std::string process, std::string module)
{
	...
	bool result = VMMDLL_Map_GetIATU(memory.vHandle, memory.GetPidFromName(process) /*| VMMDLL_PID_PROCESS_WITH_KERNELMEMORY*/, (LPSTR)module.c_str(), &iat_map);
	if (!result)
	{
...
{
	// Validate inputs before calling DMA library
...
{
	// Validate handle before executing
	if (!handle)
	{
		return;
	}
