...
public:
	...
	*/
	...
	inline std::string Read_string(std::uint64_t address)
	{
		...
		Read(string_address, buffer.data(), buffer.size());

		return std::string(buffer.data(), string_length);
	}

	/**
	 * \brief Reads a string using scatter read (optimized for batch operations)
	 * \param handle The scatter handle to use
	 * \param address The address of the string object
	 * \param out_string_length Output buffer for string length
	 * \param out_string_address Output buffer for string data address
	 * \return true if successfully added to scatter
	 */
	inline bool AddScatterReadStringRequest(VMMDLL_SCATTER_HANDLE handle, std::uint64_t address, std::int32_t* out_string_length, std::uint64_t* out_string_address)
	{
		if (!AddScatterReadRequest(handle, address + 0x10, out_string_length, sizeof(std::int32_t)))
			return false;
		if (!AddScatterReadRequest(handle, address, out_string_address, sizeof(std::uint64_t)))
			return false;
		return true;
	}

	/**
	 * \brief Reads string data using scatter read after getting string info
	 * \param handle The scatter handle to use
	 * \param string_address The address of the string data
	 * \param string_length The length of the string
	 * \param buffer Output buffer for string data
	 * \return true if successfully added to scatter
	 */
	inline bool AddScatterReadStringDataRequest(VMMDLL_SCATTER_HANDLE handle, std::uint64_t string_address, std::int32_t string_length, char* buffer)
	{
		if (string_length == 0 || string_length > 255)
			return false;
		return AddScatterReadRequest(handle, string_address, buffer, string_length);
	}

	/**
	 * \brief Reads a string using scatter read (complete operation)
	 * \param handle The scatter handle (must be executed separately)
	 * \param address The address of the string object
	 * \param out_length Output buffer for string length
	 * \param out_address Output buffer for string data address
	 * \return true if successfully added to scatter
	 */
	inline bool Read_string_scatter(VMMDLL_SCATTER_HANDLE handle, std::uint64_t address, std::int32_t* out_length, std::uint64_t* out_address)
	{
		return AddScatterReadStringRequest(handle, address, out_length, out_address);
	}

	/**
	 * \brief Helper to convert scatter string data to std::string
	 * \param string_length The length from scatter read
	 * \param string_address The address from scatter read (for strings >= 16 chars, this is the data pointer; for shorter strings, this is the string object address)
	 * \param string_object_address The original string object address (needed for inline strings)
	 * \param string_data The data buffer from scatter read (optional, if null will read from memory)
	 * \return The string or "Unknown" if invalid
	 */
	inline std::string ProcessScatterStringData(std::int32_t string_length, std::uint64_t string_address, std::uint64_t string_object_address, const char* string_data = nullptr)
	{
		if (string_length == 0 || string_length > 255)
			return "Unknown";

		// If we got the data from scatter, use it directly
		if (string_data != nullptr)
		{
			return std::string(string_data, string_length);
		}

		// Determine actual address: if length >= 16, string_address is the data pointer; otherwise, data is inline at string_object_address
		std::uint64_t actual_address = (string_length >= 16) ? string_address : string_object_address;
		
		// Fallback: read the data
		std::vector<char> buffer(string_length + 1, 0);
		Read(actual_address, buffer.data(), string_length);
		return std::string(buffer.data(), string_length);
	}
	/**
