...
DMA::DMA(bool use_memory_map) {
    ...
    char argMem[] = "-memmap";
    LPSTR argv[8] = {arg0, arg1, arg2, nullptr, nullptr, nullptr, nullptr, nullptr};
    DWORD argc = 3;
    ...
    if (use_memory_map) {
        ...
        if (!std::filesystem::exists(path) && !dump_memory_map()) {
            ...
            argv[argc++] = argMem;
            argv[argc++] = const_cast<LPSTR>(path.c_str());
        }
    }
...
DWORD DMA::get_process_id(const std::string& process_name) const {
    ...
    if (!VMMDLL_PidGetFromName(this->handle.get(), const_cast<LPSTR>(process_name.c_str()), &process_id) || process_id == 0) {
